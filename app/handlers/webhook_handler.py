...
async def handle_update(update: dict):
    ...
    if text.startswith("/start"):
        await send_message(chat_id, f"👋 Chào anh *{username}*! Em là trợ lý AI từ Thietkesim.vn. Anh cần em tư vấn gì về sim số đẹp không ạ?")
        return
    
    if text.startswith("/clear"):
        await conversation_service.clear_history(user_id)
        await send_message(chat_id, "🗑️ Đã làm mới cuộc hội thoại. Anh cần em hỗ trợ gì mới không ạ?")
        return
    ...
    try:
        ...
        await send_message(chat_id, "Dạ em Lâm đang bận chút việc, anh vui lòng nhắn lại sau ít phút hoặc để lại số điện thoại em gọi lại ngay ạ! 🙏")
